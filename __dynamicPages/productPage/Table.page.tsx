...
import Snackbars from '../../__components/snackbars';
...
const ProductTable_page = () => {
  ...
  useEffect(() => {
    if (Boolean(searchInput)) {
      ...
      setTablePage(0);
      const categoryFilter: Array<DocumentData> = prodData.filter(
        (obj: DocumentData) => obj.data().category === category,
      );
  ...
  useEffect(() => {
    ...
    if (pageLg) {
      if (prodDocLg.search === null && prodDocLg.category === null && prodData.length > 0) {
        ...
        (prodDocLg.category !== null || prodDocLg.search !== null) &&
        prodData_filtered.length > 0
      ) {
        try {
          setProdData_slice(
